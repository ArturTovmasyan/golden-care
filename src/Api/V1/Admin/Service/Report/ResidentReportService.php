...
class ResidentReportService extends BaseService
{
    ...
    public function getCovid19Report($group, ?bool $groupAll, $groupIds, $groupId, ?bool $residentAll, $residentId, $date, $dateFrom, $dateTo, $assessmentId, $assessmentFormId): SixtyDays
    {
        ...
        /** @var ResidentAdmissionRepository $admissionRepo */
        $admissionRepo = $this->em->getRepository(ResidentAdmission::class);
        $admissions = $admissionRepo->getDischarges($currentSpace, null);
        ...
        if (!empty($admissions)) {
            ...
            foreach ($admissions as $admission) {
                if ($admission->getFacilityBed() !== null) {
                    $facilityBedIds[] = $admission->getFacilityBed()->getId();
                }

                if ($admission->getApartmentBed() !== null) {
                    $apartmentBedIds[] = $admission->getApartmentBed()->getId();
                }
            }

            $facilityAdmissions = $admissionRepo->getNOtDischarges($currentSpace, null, GroupType::TYPE_FACILITY, $facilityBedIds);
            $apartmentAdmissions = $admissionRepo->getNOtDischarges($currentSpace, null, GroupType::TYPE_APARTMENT, $apartmentBedIds);

            $facilityAdmissionBedIds = [];
            $apartmentAdmissionBedIds = [];
            /** @var ResidentAdmission $facilityAdmission */
            foreach ($facilityAdmissions as $facilityAdmission) {
                if ($facilityAdmission->getFacilityBed() !== null) {
                    $facilityAdmissionBedIds[] = $facilityAdmission->getFacilityBed()->getId();
                }
            }
            /** @var ResidentAdmission $apartmentAdmission */
            foreach ($apartmentAdmissions as $apartmentAdmission) {
                if ($apartmentAdmission->getApartmentBed() !== null) {
                    $apartmentAdmissionBedIds[] = $apartmentAdmission->getApartmentBed()->getId();
                }
            }

            $finalFacilityAdmissions = [];
            $finalApartmentAdmissions = [];
            $finalFacilityBedIds = [];
            $finalApartmentBedIds = [];
            /** @var ResidentAdmission $admission */
            foreach ($admissions as $admission) {
                if ($admission->getFacilityBed() !== null && !in_array($admission->getFacilityBed()->getId(), $facilityAdmissionBedIds, false)) {
                    $finalFacilityAdmissions[] = $admission;

                    $fBedId = $admission->getFacilityBed()->getId();
                    if (array_key_exists($fBedId, $finalFacilityBedIds)) {
                        if ($admission->getStart() > $finalFacilityBedIds[$fBedId]) {
                            $finalFacilityBedIds[$fBedId] = $admission->getStart();
                        }
                    } else {
                        $finalFacilityBedIds[$fBedId] = $admission->getStart();
                    }
                ...
                if ($admission->getApartmentBed() !== null && !in_array($admission->getApartmentBed()->getId(), $apartmentAdmissionBedIds, false)) {
                    $finalApartmentAdmissions[] = $admission;

                    $aBedId = $admission->getApartmentBed()->getId();
                    if (array_key_exists($aBedId, $finalApartmentBedIds)) {
                        if ($admission->getStart() > $finalApartmentBedIds[$aBedId]) {
                            $finalApartmentBedIds[$aBedId] = $admission->getStart();
                        }
                    } else {
                        $finalApartmentBedIds[$aBedId] = $admission->getStart();
                    }
            ...
            /** @var ResidentAdmission $finalFacilityAdmission */
            foreach ($finalFacilityAdmissions as $finalFacilityAdmission) {
                $start = $finalFacilityAdmission->getStart();
                $facilityStartDate = new \DateTime($start->format('Y-m-d'));
                $facilityStartDate->setTime(0,0,0);

                $finalFacilityAdmission->setBillThroughDate($facilityStartDate);
                $this->em->persist($finalFacilityAdmission);

                if ($finalFacilityAdmission->getFacilityBed() !== null && array_key_exists($finalFacilityAdmission->getFacilityBed()->getId(), $finalFacilityBedIds)) {
                    $dates = $finalFacilityBedIds[$finalFacilityAdmission->getFacilityBed()->getId()];
                    $btDate = new \DateTime($dates->format('Y-m-d'));
                    $btDate->setTime(0,0,0);
                    $finalFacilityAdmission->getFacilityBed()->setBillThroughDate($btDate);
                    $this->em->persist($finalFacilityAdmission->getFacilityBed());
                }

            }

            /** @var ResidentAdmission $finalApartmentAdmission */
            foreach ($finalApartmentAdmissions as $finalApartmentAdmission) {
                $start = $finalApartmentAdmission->getStart();
                $apartmentStartDate = new \DateTime($start->format('Y-m-d'));
                $apartmentStartDate->setTime(0,0,0);

                $finalApartmentAdmission->setBillThroughDate($apartmentStartDate);
                $this->em->persist($finalApartmentAdmission);

                if ($finalApartmentAdmission->getApartmentBed() !== null && array_key_exists($finalApartmentAdmission->getApartmentBed()->getId(), $finalApartmentBedIds)) {
                    $dates = $finalApartmentBedIds[$finalApartmentAdmission->getApartmentBed()->getId()];
                    $btDate = new \DateTime($dates->format('Y-m-d'));
                    $btDate->setTime(0,0,0);
