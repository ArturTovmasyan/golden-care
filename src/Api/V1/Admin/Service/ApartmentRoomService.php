...
class ApartmentRoomService extends BaseService implements IGridService
{
        ...
        if (!empty($rooms)) {
            ...
            if ($vacant) {
                ...
                if (!empty($params) && !empty($params[0]['resident_id'])) {
                    ...
                    if ($lastAction !== null && $lastAction->getApartmentBed() !== null && $lastAction->getAdmissionType() === AdmissionType::DISCHARGE) {
                        $residentBedId = $lastAction->getApartmentBed()->getId();
                    }
                }
    ...
    public function getById($id)
    {
        ...
        if ($room !== null) {
            ...
            if ($beds !== null) {
                $ids = array_map(static function (ApartmentBed $item) {
                    return $item->getId();
                }, $beds->toArray());
    ...
    public function add(array $params): ?int
    {
        ...
        try {
            ...
            if ($apartmentRoom->getBeds() !== null) {
                ...
                $residentAdmissions = $admissionRepo->getBeds($currentSpace, $this->grantService->getCurrentUserEntityGrants(ResidentAdmission::class), GroupType::TYPE_APARTMENT, $disabledBedIds);
