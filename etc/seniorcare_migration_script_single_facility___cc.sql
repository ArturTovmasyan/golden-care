...
FROM `cc_old`.`residents`
       ...
       INNER JOIN `cc_old`.`facilityrooms` ON `cc_old`.`facilityrooms`.`Room_ID` = `cc_old`.`residents`.`Room_ID` AND `cc_old`.`facilityrooms`.`Facility_ID` IN (5, 9);
...
WHERE `cc_old`.`facilities`.`Facility_ID` IN (5, 9);
...
WHERE `cc_old`.`dinningroom`.`Facility_ID` IN (5, 9);
...
WHERE `cc_old`.`facilityrooms`.`Facility_ID` IN (5, 9);
